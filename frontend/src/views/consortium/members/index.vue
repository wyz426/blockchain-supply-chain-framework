  ...
  <div style="padding: 5px 20px">
    ...
    <el-form :inline="true" :model="listQuery" class="demo-form-inline">
      <el-form-item>
        ...
        <!--        />-->
        <el-form-item :label="$t('Select the blockchain')">
          <el-select v-model="blockchain" :placeholder="$t('Select the blockchain')" clearable @change="reload()">
            <el-option v-for="(block,index) in listBlockchain" :key="index" :value="block.name" />
          </el-select>
        </el-form-item>
      </el-form-item>
      <!--      <el-form-item>-->
...
export default {
  ...
  methods: {
    ...
    load() { // 显示所有表单信息 todo:url也需要改动
      axios.get(`http://localhost:8080/blockchain/member/`)
        .then(response => {
          console.log('Response data2222:', response.data);
          // 更新联盟列表
          // this.list = response.data.data
